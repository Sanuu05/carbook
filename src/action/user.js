...
export const port = isDev
  ? "http://localhost:5050"
  : "https://cariva.onrender.com";
export const loadUser = () => async (dispatch, getState) => {
  try {
    dispatch({ type: USER_LOADING });
    const token = getState().user.token;
    const { data } = await Axios.get(`${port}/user/getuser`, {
...
export const senddata = (data) => async (dispatch, getState) => {
  try {
    ...
    const token = getState().user.token;
    const dataa = await Axios.post(`${port}/main/upload`, data, {
      headers: { "x-auth-token": token },
    });
...
export const order = (data) => async (dispatch, getState) => {
  try {
    const token = getState().user.token;
    const dataa = await Axios.post(`${port}/main/order`, data, {
      headers: { "x-auth-token": token },
    });
...
export const delcar = (data) => async (dispatch, getState) => {
  try {
    const token = getState().user.token;
    // alert('del')
    const dataa = await Axios.delete(`${port}/main/car/${data}`, {
      headers: { "x-auth-token": token },
    });
...
export const myorder = (data) => async (dispatch, getState) => {
  try {
    ...
    const dataa = await Axios.get(`${port}/main/mybooking`, {
      headers: { "x-auth-token": token },
    });
...
export const carorder = (data) => async (dispatch, getState) => {
  try {
    ...
    const dataa = await Axios.get(`${port}/main/carorder`, {
      headers: { "x-auth-token": token },
    });
...
export const mycars = (data) => async (dispatch, getState) => {
  try {
    const token = getState().user.token;
    const dataa = await Axios.get(`${port}/main/mycars`, {
      headers: { "x-auth-token": token },
    });
